...
/////////////////////////////// disk commands
		...
		if( cmd_buf.dev>=0x31 && cmd_buf.dev<(0x30+DEVICESNUM) ) //D1: az D4: (ano, az od D1: !!!)
		{
...
disk_operations_direct_d0_d4:
			...
			if (cmd_buf.cmd!=0x53) FileInfo.vDisk.flags &= (~FLAGS_WRITEERROR); //vynuluje FLAGS_WRITEERROR bit
...
Send_NACK_and_set_FLAGS_WRITEERROR_and_ST_IDLE:
		   ...
		   default:
			...
			if(
			     (cmd_buf.cmd!=0x52) 
				 //&& (cmd_buf.cmd!=0x50) //uz porovnano predtimto (switch), skace na device_command_accepted;
				 && (cmd_buf.cmd!=0x53) 
				 //&& (cmd_buf.cmd!=0x57) //uz porovnano predtimto (switch), skace na device_command_accepted;
				 //&& (cmd_buf.cmd!=0x3f) //zpracovava se uvnitr teto podminky
				 && (cmd_buf.cmd!=0x4e) && (cmd_buf.cmd!=0x4f)
				 //&& (cmd_buf.cmd!=0x21) && (cmd_buf.cmd!=0x22) //uz zpracovany predtimto
			  )
			{
...
/////////////////////////////// sdrive commands
		...
		if( cmd_buf.dev == (unsigned char)0x71+3-((unsigned char)(inb(PINB))&0x03) )
		{
			...
			if ( cmd_buf.cmd<=0x53 && cmd_buf.cmd>=0x50 )	//schvalne prehozeno poradi kvuli rychlosti (pri normalnich operacich s SDrive nebude platit hned prvni cast podminky)
			{
				//(0x50,x52,0x53)
...
*/
			...
			case 0xDB:	//get vDisk flag [<1]
				{
					//atari_sector_buffer[0] = (cmd_buf.aux1<DEVICESNUM)? GetvDiskPtr(cmd_buf.aux1)->flags : FileInfo.vDisk.flags;
			...
			case 0xE2: //$E2  n ??	Deactivation of device Dn:
				...
				GetvDiskPtr(cmd_buf.aux1)->flags &= ~FLAGS_DRIVEON; //vDisk[cmd_buf.aux1].flags &= ~FLAGS_DRIVEON;
				goto Send_CMPL_and_Delay;
				break;
			...
			case 0xE3: //$E3  n ??	Set actual directory by device Dn: and get filename. 8.3 + attribute + fileindex [11+1+2=14]
			   {
				...
				if (!(GetvDiskPtr(cmd_buf.aux1)->flags & FLAGS_DRIVEON))
				{
...
Command_ED_notfound:	//sem skoci z commandu ED kdyz nenajde hledany nazev souboru
				...
				p2=((unsigned char *)&vDisk[ cmd_buf.aux1 ]);
				// vDisk <- vDisk[ cmd_buf.aux1 ]
				for(i=0; i<sizeof(virtual_disk_t); i++) p1[i]=p2[i];
			   }
